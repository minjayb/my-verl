...
# Trains repeatedly on one hard GSM8K problem (batch_size=1, each epoch = 1 step)
set -e
...
PYTHONUNBUFFERED=1 python -m verl.trainer.main_ppo \
    ...
    trainer.default_local_dir="$SCRIPT_DIR/outputs/grpo-single_$(date +%d-%m_%H%M)" \
    trainer.project_name=verl-gsm8k-grpo \
    trainer.experiment_name=single-example-gsm8k \
